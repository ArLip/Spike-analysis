...
%% Save
cd('D:\Tompouce_8thJan18\data\2016-0098-003\AD_Lev Experiment_final\analysis_Jan')
...
%WT vs TGd   saline
[hDip,pDip]=ttest2(FRmeans(drug==0&genotype==1),FRmeans(drug==0&genotype==0))

%WT vs TG -  saline - pyramidal
[hDip,pDip]=ttest2(FRmeans(drug==0&genotype==0&neuronType==2),FRmeans(drug==0&genotype==1&neuronType==2))

%WT vs TG -  saline - interneuron
[hDip,pDip]=ttest2(FRmeans(drug==0&genotype==0&neuronType==1),FRmeans(drug==0&genotype==1&neuronType==1))

%WT vs TG -  sal
[hDip,pDip]=ttest2(FRmeans(drug==0&genotype==0),FRmeans(drug==0&genotype==1))

%TG+sal vs TG+lev - interneuron
[hDip,pDip]=ttest2(FRmeans(drug==0&genotype==1&neuronType==1),FRmeans(drug==1&genotype==1&neuronType==1))
...
data{1}=FRmeans(drug==1);
data{2}=FRmeans(drug==0);
...
names={'wt - sal','tg - sal'};
ylab={'Firing Rates Hz'};
ttl={'Firing Rates for wt - tg - sal -int'};

data{1}=FRmeans(genotype==0&drug==0&neuronType==1);
data{2}=FRmeans(genotype==1&drug==0&neuronType==1);

AD_BeehivePlot(data,names,ylab,ttl)

% wt - tg - sal - int

names={'wt - sal - int','tg - sal - int'};
ylab={'Firing Rates Hz'};
ttl={'Firing Rates for wt - tg - sal - int'};

data{1}=FRmeans(genotype==0&drug==0);
data{2}=FRmeans(genotype==1&drug==0);

AD_BeehivePlot(data,names,ylab,ttl)

% wt - tg - sal - pyr

names={'wt - sal - pyr','tg - sal - pyr'};
ylab={'Firing Rates Hz'};
ttl={'Firing Rates for wt - tg - sal - pyr'};

data{1}=FRmeans(genotype==0&drug==0&neuronType==2);
data{2}=FRmeans(genotype==1&drug==0&neuronType==2);

AD_BeehivePlot(data,names,ylab,ttl)

% tg+sal vs tg+leve - int

names={'tg - sal - int','tg - leve - int'};
ylab={'Firing Rates Hz'};
ttl={'Firing Rates for tg+sal vs tg+leve - int'};

data{1}=FRmeans(genotype==1&drug==0&neuronType==1);
data{2}=FRmeans(genotype==1&drug==1&neuronType==1);
